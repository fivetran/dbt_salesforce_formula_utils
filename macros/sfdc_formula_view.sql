...
{%- set old_formula_fields = dbt_utils.get_column_values(source('salesforce', 'fivetran_formula'),'field') -%}
    ...
    select 
        {{ dbt_utils.star(source('salesforce',join_to_table_first), except=old_formula_fields) }}, --Querying the source table and excluding the old formula fields if they are present.
        {{ salesforce_formula_utils.sfdc_formula_pivot(join_to_table=join_to_table_first) }} -- Adds the results of the sfdc_formula_pivot macro as the remainder of the sql query.
