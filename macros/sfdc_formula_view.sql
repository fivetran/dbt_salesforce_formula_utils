{%- macro sfdc_formula_view(source_table, source_name='salesforce', reserved_table_name=source_table, fields_to_include=none, reference_model=none) -%}
...
{% endif %}
    ...
    {% if reference_model is not none %}
        {{ salesforce_formula_utils.sfdc_star_exact(ref(reference_model), relation_alias=(source_table + "__table"), except=current_formula_fields) }} --Querying the source table and excluding the old formula fields if they are present.
    ...
    from {{ ref(reference_model) }} as {{ source_table }}__table
    {% else %}
