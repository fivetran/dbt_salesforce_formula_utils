...
{%- set old_formula_fields = dbt_utils.get_column_values(source(source_name, 'fivetran_formula'),'field') -%}
    ...
    select 
        ...
        {{ salesforce_formula_utils.sfdc_formula_pivot(join_to_table=join_to_table_first, source_name) }} -- Adds the results of the sfdc_formula_pivot macro as the remainder of the sql query.
